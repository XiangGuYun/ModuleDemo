 ...
 * 最基类Activity
 */
abstract class AbstractActivity : SupportActivity(), BaseInterface {
    ...
    companion object {
        ...
        fun toast(text:String){
            Toast.makeText(KotlinApplication.instance.context(), text, Toast.LENGTH_SHORT).show()
        }
    }
     ...
     * 方便在Activity中弹出Toast
     */
    fun Any.toast(isLong: Boolean=false, gravity: Int = Gravity.BOTTOM, xOffset:Int=0, yOffset:Int=100){
        Toast.makeText(this@AbstractActivity, "${this}",
    ...
    fun Number.px2dp():Int{
        return DensityUtils.px2dip(this@AbstractActivity, this.toFloat())
    }

    fun Number.dp2px():Int{
        return DensityUtils.dip2px(this@AbstractActivity, this.toFloat())
    }

    fun Number.px2sp():Int{
        return DensityUtils.px2sp(this@AbstractActivity, this.toFloat())
    }

    fun Number.sp2px():Int{
        return DensityUtils.sp2px(this@AbstractActivity, this.toFloat())
    }
    ...
    fun start(allName:String, vararg pairs:Pair<String,Any>){
        val intent = Intent(this, Class.forName(allName))
        pairs.forEach {
            when(it.second){
                is String->{
